...
// }
function cpu_movement() {
  // Calculate the center position of the CPU's bat
  const cpuCenter = cpu.y + cpu.height / 2;

  // Calculate the center position of the ball
  const ballCenter = ball.y;

  // Set the speed at which the CPU's bat will move
  const cpuSpeed = 10;

  // Move the CPU's bat towards the ball's vertical position
  ...
  // Keep the CPU's bat within the canvas boundaries
  if (cpu.y < 0) {
    cpu.y = 0;
...
function updates() {
  ...
  // Collision detection for the ball with the canvas edges
  ...
  cpu_movement(); // Move the CPU's bat

  // Rest of your updates() function...
}
...
document.getElementById("start-button").addEventListener("click", function () {
  startGame();
  if (looper) clearInterval(looper); // Clear the previous interval
